...
// extra: URLUtil.JAR_SEPARATOR is missing for some reason, so we define it here
val JAR_SEPARATOR: String
    get() = "!/"
...
class CoreJrtFileSystem : DeprecatedVirtualFileSystem() {
    ...
    companion object {
        ...
        fun splitPath(path: String): Pair<String, String> {
            val separator = path.indexOf(JAR_SEPARATOR)
            if (separator < 0) {
            ...
            val pathInJar = path.substring(separator + JAR_SEPARATOR.length)
...
//                FileSystems.newFileSystem(rootUri, mapOf("java.home" to jdkHome.absolutePath))
                ConfigProvider.jrtFsProvider.newFileSystem(rootUri, mapOf("java.home" to jdkHome.absolutePath))
//            } else {
